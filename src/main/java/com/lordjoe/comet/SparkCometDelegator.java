...
/**
 * com.lordjoe.comet.SparkCometDelegator
 * instead of spreading the database we assume it is on the common file system
 * and give the whole mode to comet
 * uses calls to comet directly
 * Date: 10/7/2014
 */
public class SparkCometDelegator implements Serializable {
    ...
    public static void setMaxBinSpectra(int maxBinSpectra) {
        SparkCometDelegator.maxBinSpectra = maxBinSpectra;
    }
    ...
    public static void scoreWithFiles(String[] args) {
        ...
        JavaRDD<String> pepXMLS = spectraData.map(new CometDelegatingScoringFunction(extension,databaseName + ".fasta"));
